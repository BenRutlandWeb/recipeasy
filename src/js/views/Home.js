  ...
  <ul class="post-grid">
    <li v-for="recipe in recipes" :key="recipe.slug">
      <router-link :to="{ name:'Recipe', params: { slug: recipe.slug } }" class="card">
        <div class="img-16-9">
          <img :src="recipe.featuredImage" v-if="recipe.featuredImage" class="img-fluid" />
        </div>
        <h2 class="h6">{{ recipe.title }}</h2>
        <div>{{ recipe.excerpt }}</div>
        <p v-if="recipe.categories">{{ recipe.categories.join(', ') }}</p>
        <p v-if="recipe.tags">{{ recipe.tags.join(', ') }}</p>
      </router-link>
    </li>
  </ul>
  </div>
